...
let SubjoinContents = ({ subjoinIds, useSingleItem }) => {
	...
	return (subjoinIds.map((subjoinId, key) => {
		let subCatObj = theFoodSubjoins.find(the => the.subCatId == subjoinId);
		return (
			<div key={key} className="row" data-addtion-id={subCatObj.subCatId}>
				<label>{subCatObj.subCatName}</label>
				<hr className="my-1" />
				{subCatObj.items.map((item, key) =>
					<div key={key} className="col-auto">
						<input type={subCatObj.isMulti ? 'checkbox' : 'radio'} className="btn-check foodSubjoinOption" name={subCatObj.subCatName}
							id={subCatObj.subCatId + item.subId} value={item.subId} data-subprice={item.subPrice}
							checked={singleState.subjoinItems.indexOf("" + item.subId) >= 0}
							onChange={handleSubjoinChange}
						/>
						<label className="btn btn-pill-primary" htmlFor={subCatObj.subCatId + item.subId} >
							{item.subPrice ? (item.subName + '$' + item.subPrice) : item.subName}
						</label>
					</div>
				)
				}
...
export function ProductModal({ productId, editProduct, onClose }) {
	...
	if (productId) {
		myProductObj = theProducts.find(productObj => productObj.menuId == productId);
	} else if (editProduct) {
		let pID = editProduct.id
		flagEdit = true
		myProductObj = theProducts.find(productObj => productObj.menuId == pID);
		myProductObj = { ...myProductObj, ...editProduct }
	} else return <></>
	let { tokenId = createTokenId(), catId, menhuId, menuName, comment, price, img, isSoldOut, subjoinIds,
